...
var config = fs.readFileSync(configFile); //HARDCODE
var configData;
...
function getNodes(configData, node_array) {
    var configDataJson = JSON.parse(configData);
...
function addChildNodes(parent_node, childNodes, node_array) {
    if (childNodes) {
        for (let i = 0; i < childNodes.length; i++) {
            var child = new Node(childNodes[i].$.name, childNodes[i].$.id);
            parent_node.children.push(child);
            node_array.push(child);
            addChildNodes(child, childNodes[i].Node, node_array);
        }
    }
}
