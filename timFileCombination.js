...
var baseXML = fs.readFileSync(args[3]);
var baseData;
xmlReader.Parser().parseString(baseXML, function (err, result) {
    baseData = JSON.parse(JSON.stringify(result,null,2));
});

// console.log(baseData["TimedSequenceData"]["$"]["_dataModels"]);

// var baseDataJSON = JSON.parse(baseData);

for (let i = 4; i < args.length; i += 1) {
    var additionalDataXML = fs.readFileSync(args[i]);
    var additionalData;
...
fs.writeFileSync(args[2], newTimFile)
